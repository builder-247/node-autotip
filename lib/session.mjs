...
import { updateQueue } from './tipper.mjs';
...
class Session {
  ...
  sendKeepAlive() {
    ...
    request({
      url: `https://api.autotip.pro/keepalive?key=${key}`,
      headers,
    }, (err) => {
      if (err) {
  ...
  sendTipRequest(games = []) {
    const key = this.sessionKey;
    request({
      url: `https://api.autotip.pro/tip?key=${key}`,
      headers,
    }, (err, res, body) => {
      let JSONbody = {};
      try {
  ...
  logOut(cb) {
    request({
      url: `https://api.autotip.pro/logout?key=${this.sessionKey}`,
      headers,
    }, (err, res, body) => {
      if (err) {
