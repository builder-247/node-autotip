...
function autotipLogin(uuid, session, hash, cb) {
  request(
    `https://api.autotip.pro/login?username=${session.selectedProfile.name}&uuid=${util.removeDashes(uuid)}&tips=${getTipCount()}&v=2.1.0.6&mc=1.8.9&os=${os.type()}&hash=${hash}`,
    (err, res, body) => {
      if (err) {
        cb(err, null);
      }
