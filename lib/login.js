...
function autotipLogin(uuid, session, hash, cb) {
  getTipCount(uuid, (tipCount) => {
    request(
      {
        url: `https://autotip.sk1er.club/login?username=${session.selectedProfile.name}&uuid=${util.removeDashes(uuid)}&tips=${tipCount + 1}&v=2.1.0.6&mc=1.8.9&os=${os.type()}&hash=${hash}`,
        headers
      },
      (err, res, body) => {
        if (err) {
